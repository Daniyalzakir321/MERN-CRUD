            ...
            if (result) {
                ...
                await transporter.sendMail(mailOptions, function (error, info) {
                    if (error) {
                        ...
                        res.status(200).json({
                            message: "A confirmation email has been sent. Please Check Your Account",
...
router.post("/twilio-send-sms", async (req, res) => {
  client.messages.create({
     body: 'An otp has been sent.',
     from: '+16106863950',
     to: '+923128793647'
   })
      .then(message => {
          console.log(message.sid)
          return res.status(200).json({ message: 'Message sent successfully: ' +message.sid })
      })
      .catch(err => {
...
router.post("/send-otp", async (req, res) => {
    try {
        ...
        .then(message => console.log(message.sid)).catch(err=>{console.log("otp error",err) })
...
// TWILIO SEND SMS
router.post("/sendgrid-email", async (req, res) => {
    const sendgrid = require('@sendgrid/mail');
    const SENDGRID_API_KEY = "<SENDGRID_API_KEY>"
    sendgrid.setApiKey(SENDGRID_API_KEY)
    const msg = {
       to: 'daniyalzakir03@gmail.com',
...
//  SEND EMAIL MAIL JET
router.post("/mailjet", async (req, res) => {
    const mailjet = require ('node-mailjet')
    .connect('6a3d64e8f83ea659a695e28da006ccdb', 'a778da7f5e63e1e6e090e2f98a2a1f75')
      ...
      "Messages":[
        {
          ...
          "To": [
            {
              "Email": "daniyalzakir10@gmail.com",
              "Name": "Zayan"
            }
          ],
          "Subject": "[Verify] Email.",
          "TextPart": "My first Mailjet email",
          "HTMLPart": "<h3>HELLO FROM DANIYAL</h3>",
